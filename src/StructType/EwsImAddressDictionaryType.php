...
class EwsImAddressDictionaryType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * @var \StructType\EwsImAddressDictionaryEntryType[]
     */
    protected array $Entry = [];
    /**
     * Constructor method for ImAddressDictionaryType
     * @uses EwsImAddressDictionaryType::setEntry()
     * @param \StructType\EwsImAddressDictionaryEntryType[] $entry
     */
    ...
    public static function validateEntryForArrayConstraintsFromSetEntry(array $values = []): string
    {
        ...
        foreach ($values as $imAddressDictionaryTypeEntryItem) {
            ...
            if (!$imAddressDictionaryTypeEntryItem instanceof \StructType\EwsImAddressDictionaryEntryType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Entry property can only contain items of type \StructType\EwsImAddressDictionaryEntryType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\EwsImAddressDictionaryEntryType[] $entry
     * @return \StructType\EwsImAddressDictionaryType
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\EwsImAddressDictionaryEntryType $item
     * @return \StructType\EwsImAddressDictionaryType
     */
    public function addToEntry(\StructType\EwsImAddressDictionaryEntryType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsImAddressDictionaryEntryType) {
            throw new InvalidArgumentException(sprintf('The Entry property can only contain items of type \StructType\EwsImAddressDictionaryEntryType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->Entry[] = $item;
