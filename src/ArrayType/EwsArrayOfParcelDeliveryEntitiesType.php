...
class EwsArrayOfParcelDeliveryEntitiesType extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsParcelDeliveryEntityType) {
            throw new InvalidArgumentException(sprintf('The ParcelDelivery property can only contain items of type \StructType\EwsParcelDeliveryEntityType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
