...
class EwsNonEmptyArrayOfOccurrenceInfoType extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsOccurrenceInfoType) {
            throw new InvalidArgumentException(sprintf('The Occurrence property can only contain items of type \StructType\EwsOccurrenceInfoType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
