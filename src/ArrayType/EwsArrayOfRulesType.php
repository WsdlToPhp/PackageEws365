...
class EwsArrayOfRulesType extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsRuleType) {
            throw new InvalidArgumentException(sprintf('The Rule property can only contain items of type \StructType\EwsRuleType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
