...
class EwsArrayOfUnifiedGroupsType extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsUnifiedGroupType) {
            throw new InvalidArgumentException(sprintf('The UnifiedGroup property can only contain items of type \StructType\EwsUnifiedGroupType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
