...
class EwsArrayOfSharingInvitation extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsSharingInvitation) {
            throw new InvalidArgumentException(sprintf('The SharingInvitation property can only contain items of type \StructType\EwsSharingInvitation, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
