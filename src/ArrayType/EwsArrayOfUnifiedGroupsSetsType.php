...
class EwsArrayOfUnifiedGroupsSetsType extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsUnifiedGroupsSetType) {
            throw new InvalidArgumentException(sprintf('The UnifiedGroupsSet property can only contain items of type \StructType\EwsUnifiedGroupsSetType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
