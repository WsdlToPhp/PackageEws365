...
class EwsArrayOfMeetingSuggestionsType extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsMeetingSuggestionType) {
            throw new InvalidArgumentException(sprintf('The MeetingSuggestion property can only contain items of type \StructType\EwsMeetingSuggestionType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
