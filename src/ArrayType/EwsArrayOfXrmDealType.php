...
class EwsArrayOfXrmDealType extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsXrmDealType) {
            throw new InvalidArgumentException(sprintf('The Deal property can only contain items of type \StructType\EwsXrmDealType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
