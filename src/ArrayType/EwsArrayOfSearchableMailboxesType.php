...
class EwsArrayOfSearchableMailboxesType extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsSearchableMailboxType) {
            throw new InvalidArgumentException(sprintf('The SearchableMailbox property can only contain items of type \StructType\EwsSearchableMailboxType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
