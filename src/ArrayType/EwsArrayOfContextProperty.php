...
class EwsArrayOfContextProperty extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsContextPropertyType) {
            throw new InvalidArgumentException(sprintf('The ContextProperty property can only contain items of type \StructType\EwsContextPropertyType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
