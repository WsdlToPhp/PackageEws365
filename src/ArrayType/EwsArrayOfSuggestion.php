...
class EwsArrayOfSuggestion extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsSuggestion) {
            throw new InvalidArgumentException(sprintf('The Suggestion property can only contain items of type \StructType\EwsSuggestion, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
