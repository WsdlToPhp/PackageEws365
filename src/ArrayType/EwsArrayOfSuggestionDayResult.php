...
class EwsArrayOfSuggestionDayResult extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsSuggestionDayResult) {
            throw new InvalidArgumentException(sprintf('The SuggestionDayResult property can only contain items of type \StructType\EwsSuggestionDayResult, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
