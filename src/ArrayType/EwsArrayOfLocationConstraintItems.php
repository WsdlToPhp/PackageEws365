...
class EwsArrayOfLocationConstraintItems extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsLocationConstraintItem) {
            throw new InvalidArgumentException(sprintf('The LocationItem property can only contain items of type \StructType\EwsLocationConstraintItem, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
