...
class EwsArrayOfBookingStaffMember extends AbstractStructArrayBase
{
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EwsBookingStaffMemberType) {
            throw new InvalidArgumentException(sprintf('The StaffMember property can only contain items of type \StructType\EwsBookingStaffMemberType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
