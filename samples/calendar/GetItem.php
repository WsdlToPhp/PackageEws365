...
if ($result !== false) {
    ...
    foreach($result->getResponseMessages()->getGetItemResponseMessage() as $message) {
        /**
         * In case of a Calendar Item
         */
        if (is_array($message->getItems()->getCalendarItem())) {
            foreach($message->getItems()->getCalendarItem() as $item) {
                if ($item instanceof EwsCalendarItemType) {
                    echo PHP_EOL . sprintf('Appointment named "%s" takes place at "%s"',
                        $item->getSubject(),
